...
public class Median {
	...
	public void process(String[] inputStrArray, long[] num) {
		...
		for (int j = 0; j < length; j++) {
			...
			if (inputStrArray[j].equalsIgnoreCase("r")) {
				...
				// index = Arrays.binarySearch(intArr, num[j]);

				index = binarySearch(num[j], 0, num.length - 1, num);
				...
				if (index >= 0) {
					...
					if (intArr[-index] < num[j]) {
						...
						for (int k = length - 2; k >= -(index - 1); k--) {
	...
	public static void main(String[] args) throws IOException {
		...
		long[] arr = { -1, 3, 3, 3, 4, 6, 9, 15, 26, Integer.MAX_VALUE };

		//

		int num = 25;

		int position = med.binarySearch(num, 0, 9, arr);

		if (position >= 0) {

			if (arr[position] < num) {

				for (int k = arr.length - 2; k >= position + 1; k--) {
				...
				for (int k = arr.length - 2; k >= position; k--) {
		...
		// for (int k = position; k < arr.length - 1; k++) {
